/**
 * @file matrix_mult_dot_tiling_optimization_1_double.cu
 * 
 * CUDA code to calculate D = (A*B).*C (tiling algorithm avoid bank conflict and additional optimization, single precision)
 * 
 */
...
/** Main entry point.
 * Computes matrix multiplication using optimized tiling algorithm followed by elementwise dot product (single precision)
 */
__global__ void matrix_mult_dot_tiling_optimization_1_single( 
                      float * D, 
                      float * A, 
                      float * B,
		      float * C,
